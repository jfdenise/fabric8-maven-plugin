...
/**
 * Enriches wildfly-jar containers with health checks if bootable JAR configured for openshift.
 */
public class WildFlyJarHealthCheckEnricher extends AbstractHealthCheckEnricher {

    public WildFlyJarHealthCheckEnricher(MavenEnricherContext buildContext) {
        super(buildContext, "f8-healthcheck-wildfly-jar");
    }
    ...
    protected Probe getReadinessProbe() {
        return discoverWildFlyJarCheck(10);
    }

    @Override
    protected Probe getLivenessProbe() {
        return discoverWildFlyJarCheck(180);
    }

    private Probe discoverWildFlyJarCheck(int initialDelay) {
        MavenProject project = ((MavenEnricherContext) getContext()).getProject();
        Plugin plugin = MavenUtil.getPlugin(project, "org.wildfly.plugins", "wildfly-jar-maven-plugin");
        if (plugin == null) {
            return null;
        }
        Xpp3Dom config = (Xpp3Dom) plugin.getConfiguration();
        Xpp3Dom openshift = config.getChild("openshift");
        if (openshift != null) {
