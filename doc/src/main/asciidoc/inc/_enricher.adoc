...
The `fmp-project-label` enricher supports the following configuration options:
...
The git branch & latest commit on the branch are annotated as fabric8/git-branch & fabric8/git-commit.
...
[[f8-healthcheck-wldfly-jar]]
==== f8-healthcheck-wildfly-jar

This enricher adds kubernetes readiness and liveness probes with WildFly bootable JAR. This requires the following galleon layer
 has been enabled in the Maven plugin. TO BE REVISITED
